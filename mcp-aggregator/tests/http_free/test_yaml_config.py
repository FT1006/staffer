...
class TestYAMLConfigLoading:
    ...
    def test_yaml_config_loads_without_server_section(self):
        """RED: Test YAML config loads without server section - should fail first."""
        if not load_config:
            pytest.skip("Configuration loading not implemented yet")
        
        # Given: Valid YAML configuration WITHOUT server section (STDIO protocol)
        yaml_content = """
domain: "test_domain"
...
"""
        ...
        # NO server settings for STDIO protocol
        assert not hasattr(config, 'server') or config.server is None
